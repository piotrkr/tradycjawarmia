...
menuBtn.addEventListener('click', function () {
	...
	document.querySelectorAll('.off__screen__toggle > a').forEach(function (link) {
		link.addEventListener('click', function (e) {
			...
			const anchor = this.getAttribute('data-anchor') // np. "#aktualnosci"
			...
			if (!isActive) {
				// Pierwsze kliknięcie – rozwijamy podmenu
				toggleLi.classList.add('active')
				submenu.classList.add('active')
				const span = this.querySelector('span')
				if (span) span.textContent = '–'
			} else {
				// Drugie kliknięcie – zamykamy menu
				menuBtn.classList.remove('active')
				offScreen.classList.remove('active')

				if (anchor) {
					// Preload index.html w tle (przyspiesza przejście)
					...
					// Przenosimy na index z anchorem
...
var showPage,
	...
	totalPages,
	currentPage,
	displayedLastPageNumber,
	prevBtn,
	nextBtn,
	scrollAmount,
	announcementPages = document.querySelectorAll('.announcements-page'),
	pageNumbersContainer = document.getElementById('page-numbers'),
	paginationPrevBtn = document.getElementById('prev-btn'),
	paginationNextBtn = document.getElementById('next-btn'),
	announcementsSection = document.querySelector('#announcements'),
	newsData =
		(0 < announcementPages.length &&
			pageNumbersContainer &&
			paginationPrevBtn &&
			paginationNextBtn &&
			((showPage = function (n) {
				announcementPages.forEach(function (e, t) {
					e.hidden = t + 1 !== n
				}),
					(currentPage = n),
					document.querySelectorAll('.page-number').forEach(function (e) {
						var t = parseInt(e.dataset.page)
						e.classList.toggle('active', t === n)
					}),
					paginationPrevBtn.classList.toggle('disabled', 1 === currentPage),
					paginationNextBtn.classList.toggle('disabled', currentPage === totalPages),
					announcementsSection && announcementsSection.scrollIntoView({ behavior: 'smooth', block: 'start' })
			}),
				...
				for (
					var e = function (e) {
							...
							;(t.className = 'page-number'),
								...
								e === currentPage && t.classList.add('active'),
								pageNumbersContainer.appendChild(t)
						},
						t = 1;
					t <= Math.min(3, totalPages);
					t++
				)
					e(t)
				4 < totalPages &&
					(((n = document.createElement('span')).className = 'page-number dots'),
					(n.textContent = '...'),
					pageNumbersContainer.appendChild(n))
				var n = Math.max(totalPages, displayedLastPageNumber)
				3 < totalPages && 3 < n && e(totalPages)
			}),
			(totalPages = announcementPages.length),
			(currentPage = 1),
			(displayedLastPageNumber = 10),
			paginationPrevBtn.addEventListener('click', function () {
				1 < currentPage && showPage(currentPage - 1)
			}),
...
newsGrid &&
	...
	(prevBtn = document.querySelector('.news-carousel__prev')),
	(nextBtn = document.querySelector('.news-carousel__next')),
	(scrollAmount = 370),
